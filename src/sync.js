...
import { getSent, replaceSent } from './sent';
...
/** Pushes the current local friends/lists/sent history up to Drive, creating the file on first use. */
export async function pushToDrive() {
  const data = { updatedAt: new Date().toISOString(), friends: getFriends(), lists: getLists(), sent: getSent() };
...
/**
 * Runs once after sign-in. Recovers friends/lists/sent history from Drive
 * when this device's local storage is empty (e.g. iOS wiped an inactive
 * PWA's storage), or when Drive holds a newer copy than this device has
 * seen; otherwise pushes local data up so Drive stays current.
...
export async function hydrateFromDrive() {
  ...
  const localHasData = getFriends().length > 0 || getLists().length > 0 || getSent().length > 0;
